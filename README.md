...
4. Switch the default input in Sound settings (or put AirPods on / take them off): the name should update, playback should follow the new output, and both the system tap and the mic should keep capturing. Bluetooth connect/disconnect is detected from the device list plus the default input/output. If no mic remains, bars stay still and a warning appears. Putting AirPods on or taking them off does not block **Generate** — already-captured audio is kept, and a hung mic rebind times out so Stop/Generate can still write the mix. The **HH:MM:SS** clock stays at zero until the first sound, then runs. **Stop** pauses both sources and the clock; the button becomes **Resume**. **Generate** stays off until that clock reaches **`minListenSeconds`** from `GET /config` (`MIN_LISTEN_SECONDS` in backend `.env`). Then it transcribes the mix and stops capture. **Move to trash** discards the buffer and returns to the notepad.
