...
**+ New note** (orange, always in the top bar) opens the original capture spike and **starts recording immediately**. The spike uses the same cream page and pill buttons as the notepad. The screen title is an editable **New note** field. Below it is a borderless notes field with a subtle **Write notes...** / **Escribe tus notas...** placeholder. Controls sit at the bottom: meter → **Stop** / **Resume** → **Generate** → clock. The notepad and New note screens share the same window size. **Stop** pauses; the same button then says **Resume**. **Generate** stays off while the mix is silent. The clock starts on the first mic or system sound, and Generate turns on after **`minListenSeconds`** from `GET /config` (`MIN_LISTEN_SECONDS` in backend `.env`). Then it transcribes the mix and writes it into that same notes field (**My notes:** and typed notes first, then **Transcript:** and the transcript; **Transcript:** and the transcript only if there were no notes). The field stays scrolled to the top. There is no separate transcript box. A subtle **Home** control returns to the notepad. Top-right **...** → **Move to trash** discards an in-progress listen and returns to the notepad. Sign-in and language stay on **Profile**.
...
1. Open **+ New note**. Recording starts automatically. The level pill sits to the left of **Stop**. Allow **Microphone**. Allow **Screen Recording** — that permission is for **meeting sound** (Zoom / Meet / Teams / YouTube / Netflix and anything else the Mac plays), not to save video or screenshots. Capture is **audio only**: **AVAudioEngine** (mic) + a **global Core Audio process tap** (system playback). We do **not** use ScreenCaptureKit and we never save video. The tap is never bound to the headphone device (that would mute apps). Putting AirPods on or taking them off **releases** the tap so playback can follow the new device, then starts a new global tap. Already-captured audio is kept and time-aligned. The mic follows the macOS default input (AirPods, USB, or the Mac’s built-in, including the 3-mic array). YouTube, Netflix, Meet, and other apps mix together; switching tabs or apps does not restart capture. DRM video can stay visible. The headphone jack is never treated as a mic.
...
4. Switch the default input in Sound settings (or put AirPods on / take them off): the name should update, playback should follow the new output, and both the system tap and the mic should keep capturing. Bluetooth connect/disconnect is detected from the device list plus the default input/output. If no mic remains, bars stay still and a warning appears. Putting AirPods on or taking them off does not block **Generate** — the mix is copied the moment you click Generate, so a Bluetooth handshake cannot empty the buffer. A hung mic rebind times out, and Generate during connect/disconnect still transcribes audio so far. The **HH:MM:SS** clock stays at zero until the first sound, then runs. **Stop** pauses both sources and the clock; the button becomes **Resume**. **Generate** stays off until that clock reaches **`minListenSeconds`** from `GET /config` (`MIN_LISTEN_SECONDS` in backend `.env`). Then it transcribes the mix and stops capture. **Move to trash** discards the buffer and returns to the notepad.
5. Play something in headphones and say a short phrase, then **Generate**.
6. The mix (16-bit PCM WAV, mono, 16 kHz, `fmt`  + `data` only) is posted to `POST /spike/transcribe` as **time-ordered clips**: system playback and mic are sliced where each is loudest (YouTube → you → Netflix stays in that order). That also avoids sending each full track twice, which made Generate slow. Long clips are still split on the server into parts of `WAV_CHUNK_SECONDS` (default 10 minutes, under OpenAI’s 25 MB per-file limit). The combined **spoken** transcript is written into the notes field **after Generate** (**My notes:** first if you typed anything, then **Transcript:**). STT instruction prompts are not shown. **Generate** stops the mic and system-audio tap — wait until you have finished speaking, then Generate **once**. Frontend and API terminals print segment times and STT previews for debugging. The **API terminal** also prints an estimated STT cost in USD from the transcription `usage` OpenAI returns, times the current list price on that model’s OpenAI docs page (fetched at runtime, not stored in code or `.env`). That estimate is not shown in the app.
...
**Stop** / **Resume** pauses or continues both sources. **Generate** stays off until the first sound, then until `minListenSeconds` from `GET /config`. **Move to trash** discards the buffer and returns to the notepad. Restarting the Mac app clears the last mix.
...
| **Headphones (AirPods / Bluetooth), YouTube + own voice** | Re-test: Bluetooth connect/disconnect switches mic + output; global tap is released then recreated; mix keeps the whole listen. |
